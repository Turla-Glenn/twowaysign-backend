...
try:
    ...
    print("Error: 'en_US' dictionary not found.  Install enchant dictionaries.")
    exit()
...
try:
    ...
    print("Make sure 'cnn8grps_rad1_model.h5' is in the correct directory.")
    exit()

offset = 29

# Global variables
str_sentence = ""  # Initialize to empty string
prev_char = ""
ten_prev_char = [""] * 10  # Initialize to empty strings
count = -1
...
def predict_letter(test_image, pts):
    white = test_image
    ...
    pl = [ch1, ch2]
    if pl in l:
        if not (pts[0][0] + 13 < pts[8][0] and pts[0][0] + 13 < pts[12][0] and pts[0][0] + 13 < pts[16][0] and
                pts[0][0] + 13 < pts[20][0]) and not (
...
def predict():
    global str_sentence, prev_char, ten_prev_char, count, last_predicted_letter
    try:
        data = request.get_json()
        frame_data = data['frame']
        frame_bytes = base64.b64decode(frame_data)
        frame_buffer = BytesIO(frame_bytes)
        img = Image.open(frame_buffer)
        ...
        if hands:
            ...
            if image.size > 0:
                ...
                if handz:
                    ...
                    predicted_action = predict_letter(white, pts)

                    if predicted_action == "next":
                        str_sentence += last_predicted_letter
                        str_sentence += " "
                        last_predicted_letter = ""
                    elif predicted_action == "backspace":
                        str_sentence = str_sentence[:-1]
                        last_predicted_letter = ""
                    elif predicted_action:
                        last_predicted_letter = predicted_action

                    response_data = {
                        'sentence': str_sentence,
                        'letter': predicted_action,
                        'last_letter': last_predicted_letter
                    }
                    print(f"Response: {response_data}")  # Add this line for debugging
...
def reset():
    global str_sentence, last_predicted_letter
    str_sentence = ""
    last_predicted_letter = ""

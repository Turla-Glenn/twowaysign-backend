...
def predict_letter(test_image, pts):  # pts as argument
    white = test_image
...
def predict():
    global str_sentence, prev_char, ten_prev_char, count
    try:
        data = request.get_json()
        frame_data = data['frame']  # Frame data as base64 string
        frame_bytes = base64.b64decode(frame_data)
        frame_buffer = BytesIO(frame_bytes)
        img = Image.open(frame_buffer)
        frame = cv2.cvtColor(np.array(img), cv2.COLOR_RGB2BGR)

        hands = hd.findHands(frame, draw=False, flipType=True)  # No flipType for non-mirrored
        if hands:
            hand = hands[0][0]
            x, y, w, h = hand['bbox']
            ...
            if image.size > 0:
                ...
                if handz:
                    handmap = handz[0][0]
                    pts = handmap['lmList']  # Define pts here
                    white = np.ones((400, 400, 3), np.uint8) * 255  # White background
                    os = ((400 - w) // 2) - 15
                    os1 = ((400 - h) // 2) - 15

                    # Draw hand landmarks and lines on the white image
                    ...
                    predicted_letter = predict_letter(white, pts)  # Pass pts
...
def update_sentence():
    global str_sentence
    data = request.get_json()
    str_sentence = data['sentence']  # Update the global sentence
